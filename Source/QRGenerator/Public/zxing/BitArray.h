...
public:
	...
	void set(int i, bool val) { _bits.at(i) = static_cast<uint8_t>(val); }
	...
	*/
	...
	{
		for (; numBits; --numBits)
			_bits.push_back(static_cast<uint8_t>((value >> (numBits-1)) & 1));
	}

	void appendBit(bool bit) { _bits.push_back(static_cast<uint8_t>(bit)); }
...
// Inline minimal implementations to avoid separate compilation units
inline void BitArray::bitwiseXOR(const BitArray& other)
{
	if (other.size() != size())
		throw std::invalid_argument("BitArray::bitwiseXOR(): sizes differ");
	for (int i = 0; i < size(); ++i)
		_bits[static_cast<size_t>(i)] = static_cast<uint8_t>(_bits[static_cast<size_t>(i)] ^ other._bits[static_cast<size_t>(i)]);
}

inline ByteArray BitArray::toBytes(int bitOffset, int numBytes) const
{
	if (bitOffset < 0 || bitOffset > size())
		throw std::out_of_range("BitArray::toBytes(): bitOffset out of range");
	int remainingBits = size() - bitOffset;
	int outBytes = numBytes >= 0 ? numBytes : (remainingBits + 7) / 8;
	ByteArray out(outBytes);
	int bitIndex = bitOffset;
	for (int i = 0; i < outBytes; ++i) {
		int val = 0;
		for (int b = 0; b < 8; ++b) {
			val <<= 1;
			if (bitIndex < size() && get(bitIndex))
				val |= 1;
			++bitIndex;
		}
		out[static_cast<size_t>(i)] = static_cast<uint8_t>(val);
	}
	return out;
}
